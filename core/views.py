...
def dashboard(request):
    ...
    if profile.linkedin_token:
        try:
            resp = requests.get("https://api.linkedin.com/v2/userinfo", headers={"Authorization": f"Bearer {profile.linkedin_token}"}, timeout=10)
            if resp.status_code == 200:
                user_info = resp.json()
                linkedin_data = {"name": user_info.get("name"), "email": user_info.get("email"), "picture": user_info.get("picture"), "sub": user_info.get("sub")}
            else:
    ...
    if profile.twitter_access_token:
        try:
            user_resp = requests.get("https://api.twitter.com/2/users/me?user.fields=profile_image_url,public_metrics,description,created_at", headers={"Authorization": f"Bearer {profile.twitter_access_token}"}, timeout=10)
    ...
    if profile.reddit_access_token:
        try:
            headers = {"Authorization": f"Bearer {profile.reddit_access_token}", "User-Agent": "SocialMediaDashboard/1.0"}
    ...
    if profile.youtube_token:
        try:
            yt_resp = requests.get("https://www.googleapis.com/youtube/v3/channels?part=snippet,statistics&mine=true", headers={"Authorization": f"Bearer {profile.youtube_token}", "Accept": "application/json"}, timeout=10)
    ...
    if profile.snapchat_access_token:
        try:
            headers = {"Authorization": f"Bearer {profile.snapchat_access_token}", "Content-Type": "application/json"}
    ...
    if profile.facebook_token:
        try:
            fields = ",".join([
                "id","name","email","birthday","age_range","gender","link","hometown","location",
                "friends.limit(0).summary(true)","likes.limit(0).summary(true)",
                "photos.limit(0).summary(true)","videos.limit(0).summary(true)","posts.limit(0).summary(true)"
            ])
            url = f"https://graph.facebook.com/me?fields={fields}&access_token={profile.facebook_token}"
            r = requests.get(url, timeout=10)
            r.raise_for_status()
            data = r.json()
            if "error" in data:
                facebook_data = {"error": data["error"].get("message", "Failed to fetch data")}
                facebook_graphs = []
            else:
            ...
            facebook_graphs = [
                {"metric": "Friends", "value": facebook_data["friends_count"], "height": facebook_data["friends_count"] * 0.5},
                {"metric": "Likes", "value": facebook_data["likes_count"], "height": facebook_data["likes_count"] * 0.5},
                {"metric": "Photos", "value": facebook_data["photos_count"], "height": facebook_data["photos_count"] * 2},
                {"metric": "Videos", "value": facebook_data["videos_count"], "height": facebook_data["videos_count"] * 2},
                {"metric": "Posts", "value": facebook_data["posts_count"], "height": facebook_data["posts_count"] * 2},
            ]
        except Exception as e:
    ...
    return render(request, 'dashboard.html', {
        ...
        'facebook_graphs': facebook_graphs,
    })
...
def facebook_auth_start(request):
    ...
    auth_url = f"https://www.facebook.com/v17.0/dialog/oauth?client_id={client_id}&redirect_uri={urllib.parse.quote(redirect_uri)}&scope={scope}&response_type=code"
    return redirect(auth_url)