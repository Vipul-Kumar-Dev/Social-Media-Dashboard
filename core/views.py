...
def profile(request):
    ...
    if profile.reddit_access_token:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                reddit_data = {"error": f"⚠️ Reddit error: {resp.status_code}"}
        except Exception as e:
    ...
    if profile.youtube_token:
        ...
        try:
            ...
            print("▶️ YouTube API Status Code:", yt_resp.status_code)
            print("▶️ YouTube API Raw Response:", yt_resp.text)
            ...
            if yt_resp.status_code == 200:
                ...
                if "items" in data and data["items"]:
                    ...
                    youtube_data = {
                        ...
                        "view_count": item["statistics"].get("viewCount", 0),
                        "video_count": item["statistics"].get("videoCount", 0),
                        "top_videos": []  # optional enhancement
                    }
                else:
...
def send_otp(request):
    ...
    profile.otp_temp_phone = phone  # 📌 new temp phone
    profile.save()
...
def reset_password_view(request):
    if request.method == 'POST':
        ...
        # Make sure you saved user email in session after verifying OTP
...
def linkedin_callback(request):
    ...
    try:
        ...
        # Decode JWT without verifying signature just to read data
        payload = jwt.decode(id_token, options={"verify_signature": False})
        email = payload.get("email")
        sub = payload.get("sub")
        name = payload.get("name")

        # Save token and mark as connected
        profile = request.user.profile
...
def twitter_login(request):
    # Remove ONLY old twitter state to be safe
...
def reddit_callback(request):
    ...
    client_id = "0Pm-JYkRtHID1mXZkIEK7Q"
    secret = "ujO7Kt5YO8vMxcqUiy5pM_LYWAEhVQ"
    redirect_uri = "http://localhost:8000/reddit/callback/"
    ...
    headers = {"User-Agent": "django:test:v1.0 by Kunal"}
    ...
    user_resp = requests.get(
        ...
        headers={
            ...
            "User-Agent": "django:test:v1.0 by Vipul"
        }
    )
    user_data = user_resp.json()
    ...
    try:
        ...
        if refresh_token:  # Only present on first auth
            profile.reddit_refresh_token = refresh_token
        profile.save()
        messages.success(request, "Reddit account linked successfully!")
    except Exception as e:
