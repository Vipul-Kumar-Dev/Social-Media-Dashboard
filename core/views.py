...
def pinterest_callback(request):
    code = request.GET.get("code")
    if not code:
        messages.error(request, "No authorization code received from Pinterest.")
        return redirect("profile")
    data = {
        "code": code,
        "client_id": settings.PINTEREST_CLIENT_ID,
    ...
    token_response = requests.post("https://api.pinterest.com/v5/oauth/token", json=data)
    token_data = token_response.json()
    access_token = token_data.get("access_token")
    refresh_token = token_data.get("refresh_token")
    if not access_token:
        messages.error(request, f"Pinterest Token Error: {token_data}")
        return redirect("profile")
    try:
        profile = request.user.profile
        profile.pinterest_access_token = access_token
        profile.pinterest_refresh_token = refresh_token
        profile.pinterest_token_updated_at = timezone.now()
        profile.save()
        messages.success(request, "Pinterest account linked successfully!")
    except Exception as e:
        messages.error(request, f"Error saving Pinterest token: {e}")
    return redirect("profile")
def refresh_pinterest_token(user):
    profile = user.profile
    refresh_token = profile.pinterest_refresh_token
    ...
    response = requests.post("https://api.pinterest.com/v5/oauth/token", data=data)
    token_data = response.json()
    if "access_token" in token_data:
        profile.pinterest_access_token = token_data["access_token"]
        profile.pinterest_refresh_token = token_data.get("refresh_token", refresh_token)
        profile.pinterest_token_updated_at = timezone.now()
        profile.save()
        return True
    return False
def fetch_pinterest_account_data(user):
    profile = user.profile
    if profile.pinterest_token_updated_at and timezone.now() - profile.pinterest_token_updated_at > timedelta(minutes=50):
        refreshed = refresh_pinterest_token(user)
        if not refreshed:
            return {"error": "Pinterest token expired and refresh failed."}
    access_token = profile.pinterest_access_token
    if not access_token:
        return {"error": "No Pinterest access token found."}
    headers = {
        "Authorization": f"Bearer {access_token}",
        "Accept": "application/json",
    }
    account_resp = requests.get("https://api.pinterest.com/v5/user_account", headers=headers)
    if account_resp.status_code != 200:
        try:
            return {"error": account_resp.json().get("message", "Failed to fetch Pinterest data")}
        except Exception:
            return {"error": "Unable to fetch Pinterest data"}
